...
- 회원가입 / 로그인 / 로그아웃 (세션 기반, bcrypt 해시, 로그인 실패 잠금)
- 마이페이지 (소개글/비밀번호 수정, 내 상품 관리, 거래 내역 조회) / 타인 프로필 조회(최소 정보만)
- 상품 등록(사진 필수)/수정/삭제, 검색, 판매 상태 관리(판매중/예약중/거래완료)
- **상품 구매**("구매하기" 버튼) — 아래 [상품 구매 흐름](#상품-구매-흐름) 참고
- 전체 채팅 + 상품별 1:1 채팅 (Socket.IO 실시간)
- 신고(상품/유저), 신고 누적 시 자동 차단/휴면 처리
- 송금(빠른 금액 버튼) + 잔액 모의 충전, 거래 내역(`/mypage/transactions`)에서 송금/충전/구매/판매 조회
- 관리자 페이지 (유저/상품/신고 관리, role 기반 접근 제어)

### 상품 구매 흐름

상품 상세 페이지의 "구매하기" 버튼은 본인 상품이거나 `sale_status`가 `on_sale`이 아니면 아예 보이지 않습니다. 버튼을 누르면 서버에서 다음을 **하나의 트랜잭션으로 원자적으로** 처리합니다:

1. 구매자 잔액이 상품 가격 이상인지 확인 (부족하면 거부)
2. `UPDATE products SET sale_status='sold', buyer_id=? WHERE id=? AND sale_status='on_sale'` 형태의 **조건부 UPDATE**로 상품 상태를 변경 — 두 사람이 거의 동시에 같은 상품을 구매해도 실제로 반영되는(영향받은 row가 1개인) 쪽만 성공하고, 나머지는 "이미 판매된 상품입니다"로 즉시 거부됩니다(조회 후 판단이 아니라 이 UPDATE 자체가 원자적 잠금 역할을 함)
3. 구매자 → 판매자로 상품 가격만큼 송금(기존 송금 검증 로직 재사용, 판매자의 잔액 상한 1,000만원도 재검증)
4. 구매자-판매자 간 채팅 스레드(`ChatThread`)를 자동 생성 — 이미 문의하며 대화한 스레드가 있으면 그대로 재사용

위 단계 중 하나라도 실패하면 전부 rollback되어 잔액/상품 상태 어느 쪽도 부분적으로 반영되지 않습니다. 구매/판매 내역은 마이페이지의 "거래 내역"(`/mypage/transactions`)에서 일반 송금·충전과 구분되어("OOO님에게서 [상품명] 구매" / "OOO님에게 [상품명] 판매") 표시됩니다.
...
- 회원가입/로그인/세션, 마이페이지, 상품 CRUD(+사진 검증), 상품 검색, 채팅(전체+1:1, Socket.IO), 신고+자동차단, 송금+모의충전+거래내역 조회, 관리자, 프로필 조회, 판매 상태 변경까지 기능별 테스트 파일로 나뉘어 있으며 총 133개입니다(`tests/test_*.py`).
...
- ✅ 상품 구매 — 판매자 본인 구매는 버튼 비노출 + 서버측 403 이중 방어, 동시 구매는 조건부 `UPDATE ... WHERE sale_status='on_sale'`로 방지(실제 스레드 2개로 동시 요청 보내 정확히 1명만 성공하는 것 확인), 잔액부족/이미판매됨/예약중/판매자 잔액상한 초과 시 송금까지 전부 rollback
...
- ✅ 거래 내역 조회(`/mypage/transactions`) — 항상 `current_user.id`로만 필터링해 본인 송금/충전 내역만 노출(IDOR 방지), 타인 내역 비노출 테스트로 확인
